...
import (
	...
	"github.com/AmadlaOrg/enjoin-certificate/certificate"
...
const (
	appName = "enjoin-certificate"
	version = "1.0.0"
)

var rootCmd = &cobra.Command{
	Use:     appName,
	Short:   "Enjoin plugin for TLS/SSL certificate management",
	Version: version,
}
	...
	infoCmd = &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			metadata := map[string]any{
				...
				"version":     version,
				"engine":      "certificate",
				"supports":    []string{"amadla.org/entity/security/certificate@^v1.0.0"},
				"description": "Manages TLS/SSL certificates via ACME, self-signed, or manual placement",
			}
...
var (
	...
	applyCmd = &cobra.Command{
		Use:   "apply",
		Short: "Apply certificate configuration",
		RunE:  runApply,
	}

	validateCmd = &cobra.Command{
		Use:   "validate",
		Short: "Validate certificate configuration without changes",
		RunE:  runValidate,
	}
)
...
func parseInput(data []byte) (*certificate.Config, error) {
	trimmed := strings.TrimSpace(string(data))
	var cfg certificate.Config
...
func runValidate(cmd *cobra.Command, args []string) error {
	...
	result := certificate.Validate(cfg)
	enc := json.NewEncoder(os.Stdout)
	enc.SetIndent("", "  ")
	if err := enc.Encode(result); err != nil {
		fmt.Fprintf(os.Stderr, "error encoding output: %v\n", err)
